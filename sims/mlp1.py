...
class TextBox:
    ...
    def handle_event(self, event):
        if event.type == pygame.MOUSEBUTTONDOWN:
            # Activa el textbox si se hace clic dentro; caso contrario se desactiva
...
def generate_points(dataset_type, n=100):
    ...
    if dataset_type == "lineal":
        ...
        # Por ejemplo: y = 0.03*(x-30)^2 + 1 + ruido
        for _ in range(n):
            x = random.uniform(0, 100)
...
def data_to_screen(x, y):
    # Suponemos que los datos están en el rango [0, 100] para x e y
    x_screen = PLOT_LEFT + (x / 100) * PLOT_WIDTH
    y_screen = PLOT_TOP + PLOT_HEIGHT - (y / 100) * PLOT_HEIGHT
    return int(x_screen), int(y_screen)

def draw_model_curve(surface):
    """ Dibuja la función aprendida por la red neuronal sobre el rango de x de 0 a 100 """
    puntos = []
    # Usamos una resolución de 100 puntos (se puede ajustar)
    for i in range(101):
        x = i  # x en datos
        y = nn_forward(x)
...
# Solamente usamos un slider para el learning rate
slider_lr = Slider(550, 630, 200, 20, 0.000001, 0.001, 0.00001, label="Learning Rate")

# Cuadros de texto para configurar pasos por segundo y número de puntos
...
while running:
    ...
    # Actualización automática de GD en modo Play/Pause
    if play_mode:
        accumulated_time += dt
        steps_per_sec = textbox_steps.get_value()
        # Ejecutar un paso de GD cada 1 / steps_per_sec segundos
    ...
    # Dibujar la función aprendida por la red neuronal (curva en rojo)
    draw_model_curve(screen)

    # Dibujar elementos de UI: botones, slider y textboxes
