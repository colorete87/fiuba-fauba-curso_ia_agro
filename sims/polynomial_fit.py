...
pygame.display.set_caption("Advanced Polynomial & Function Fitting Visualization")
...
class FitFunctionSelector:
    def __init__(self, x, y, width, height):
        self.rect = pygame.Rect(x, y, width, height)
        self.selected = 0
        self.options = ["Linear", "Quadratic", "Sine Wave"]
        self.font = pygame.font.SysFont('Arial', 16)
    ...
    def draw(self, surface):
        ...
        text = f"Fit: {self.options[self.selected]}"
        text_surf = self.font.render(text, True, COLORS['dark'])
        text_rect = text_surf.get_rect(center=self.rect.center)
        surface.blit(text_surf, text_rect)
...
def predict_value(x, fit_type, params):
...
def compute_error(points, fit_type, params):
    mse = 0
    n = len(points)
    for (x, y) in points:
        pred = predict_value(x, fit_type, params)
        mse += (pred - y) ** 2
    return mse / n if n > 0 else 0

def compute_r_squared(points, fit_type, params):
    y_mean = sum(y for _, y in points) / len(points)
    ss_tot = sum((y - y_mean) ** 2 for _, y in points)
    ss_res = sum((y - predict_value(x, fit_type, params)) ** 2 for x, y in points)
    return 1 - (ss_res / ss_tot) if ss_tot > 0 else 0

def gradient_descent_step(points, fit_type, params, lr):
...
def update_slider_visibility(fit_type, sliders):
    ...
    if fit_type == 0:  # Linear: a, b
        ...
        sliders[2].visible = False  # a_quad
        sliders[3].visible = False  # b_quad
        sliders[4].visible = False  # c_quad
        sliders[5].visible = False  # A_sine
        sliders[6].visible = False  # f_sine
        sliders[7].visible = False  # p_sine
        sliders[8].visible = False  # bias_sine
    elif fit_type == 1:  # Quadratic: a, b, c
        sliders[0].visible = False  # a_linear
        sliders[1].visible = False  # b_linear
...
# Estado inicial
dataset_type = 0
fit_type = 0  # 0: linear, 1: quadratic, 2: sine
...
# Elementos de UI
dataset_selector = DatasetSelector(50, 520, 200, 40)
fit_selector = FitFunctionSelector(270, 520, 200, 40)
button_step = ModernButton(490, 520, 120, 40, "Step", COLORS['success'])
button_play = ModernButton(630, 520, 120, 40, "Play", COLORS['info'])
button_reset = ModernButton(770, 520, 120, 40, "Reset", COLORS['warning'])
button_auto_fit = ModernButton(910, 520, 120, 40, "Auto Fit", COLORS['danger'])

# Sliders para diferentes tipos de ajuste
# Linear fit sliders
slider_a_linear = ModernSlider(50, 680, 200, 20, -2, 2, 0.0, "Slope (a)", COLORS['primary'])
slider_b_linear = ModernSlider(270, 680, 200, 20, -50, 150, 0.0, "Intercept (b)", COLORS['secondary'])

# Quadratic fit sliders
slider_a_quad = ModernSlider(50, 680, 200, 20, -0.1, 0.1, 0.0, "a (x²)", COLORS['primary'])
slider_b_quad = ModernSlider(270, 680, 200, 20, -2.0, 2.0, 0.0, "b (x)", COLORS['secondary'])
slider_c_quad = ModernSlider(490, 680, 200, 20, -50.0, 150.0, 0.0, "c (const)", COLORS['info'])

# Sine fit sliders
slider_A_sine = ModernSlider(50, 680, 200, 20, -50, 50, 30.0, "Amplitude (A)", COLORS['primary'])
slider_f_sine = ModernSlider(270, 680, 200, 20, 0, 0.5, 0.1, "Frequency (f)", COLORS['secondary'])
slider_p_sine = ModernSlider(490, 680, 200, 20, -50, 50, 0.0, "Phase (p)", COLORS['info'])
slider_bias_sine = ModernSlider(710, 680, 200, 20, -100, 100, 50.0, "Bias", COLORS['warning'])

slider_lr = ModernSlider(50, 610, 200, 20, 0.000001, 0.001, 0.0001, "Learning Rate", COLORS['warning'])
# slider_lr.log_scale = True  # Enable logarithmic scale for learning rate
slider_noise = ModernSlider(270, 610, 200, 20, 0, 20, 8, "Noise Level", COLORS['danger'])

# Log scale button for cost function
button_log_scale = ModernButton(970, 10, 150, 30, "Linear Scale", COLORS['secondary'])

# TextBoxes
textbox_steps = ModernTextBox(490, 600, 80, 30, "4", "Steps/sec")
textbox_n_points = ModernTextBox(630, 600, 80, 30, "100", "Points")
...
while running:
    ...
    for event in pygame.event.get():
        ...
        fit_selector.handle_event(event)
        ...
        if button_step.handle_event(event):
            ...
            cost_history.append(compute_error(points, fit_type, params))
        ...
        # Cambio de función de ajuste
        if fit_selector.selected != fit_type:
            fit_type = fit_selector.selected
            update_slider_visibility(fit_type, all_sliders)
            cost_history = []
    ...
    if play_mode:
        ...
        while accumulated_time >= 1.0 / steps_per_sec:
            ...
            # Update sliders with new parameter values
            if fit_type == 0:  # Linear
                slider_a_linear.set_value(params[0])
                slider_b_linear.set_value(params[1])
            elif fit_type == 1:  # Quadratic
    ...
    if len(points) > 0:
        ...
        for x in range(0, 101, 2):
            y = predict_value(x, fit_type, params)
    ...
    # Dibujar gráfico de coste (mismo código que antes)
    ...
    screen.blit(fit_text, (stats_rect.x + 10, stats_rect.y + 55))
    ...
    screen.blit(params_text, (stats_rect.x + 10, stats_rect.y + 75))
    
    # Separator line
    pygame.draw.line(screen, COLORS['gray_300'], (stats_rect.x + 10, stats_rect.y + 95), 
                     (stats_rect.x + stats_rect.width - 10, stats_rect.y + 95), 1)
    
    mse_text = text_font.render(f"MSE: {error_value:.4f}", True, COLORS['dark'])
    screen.blit(mse_text, (stats_rect.x + 10, stats_rect.y + 105))
    
    r2_text = text_font.render(f"R²: {r_squared:.4f}", True, COLORS['dark'])
    screen.blit(r2_text, (stats_rect.x + 10, stats_rect.y + 125))
    
    steps_text = text_font.render(f"Steps: {len(cost_history)}", True, COLORS['dark'])
    screen.blit(steps_text, (stats_rect.x + 10, stats_rect.y + 145))
    
    noise_text = text_font.render(f"Noise: {current_noise_level:.1f}", True, COLORS['dark'])
    screen.blit(noise_text, (stats_rect.x + 10, stats_rect.y + 165))

    # Títulos
    main_title = title_font.render("Polynomial & Function Fitting Visualization", True, COLORS['dark'])
